            ...
            <div class="card-body">
               ...
               <form action="{{route('lab.store')}}" method="post" enctype="multipart/form-data">
                  ...
                  <div class="row">
                     ...
                     <div class="form-group col-6 mb-3 {{ $errors->has('pincode') ? 'has-error' : '' }}">
    <label class="form-label" for="pincode">{{ __('Pincode') }}<span class="text-danger">*</span></label>
    <select name="pincode[]" id="pincode" class="select2 form-control pincode" multiple="multiple">
    @foreach($pincodes ?? [] as $item)
        <option value="{{ $item->id }}" 
            {{ isset($updates->pincodes) && in_array($item->id, $updates->pincodes) ? 'selected' : '' }}>
            {{ $item->pincode }}
        </option>
    @endforeach
</select>
...
$(document).ready(function() {
    $('#pincode').select2({
        placeholder: 'Select or Add Pincodes',
        tags: true,  // Allows adding new pincode entries
        ajax: {
            url: '/pincode/autocomplete',  // Update this with the correct route
            dataType: 'json',
            delay: 250,
            ...
            processResults: function (data) {
                return {
                    results: $.map(data, function (item) {
                        return {
                            id: item.id,
                            text: item.pincode // Assuming you are returning pincode data from your controller
                        }
                    })
                };
            },
